const r = requireFreshCompileNodeRaylibWrapper();
...
while (!r.WindowShouldClose()) {
  ...
  r.DrawText('Congrats! You created your first node-raylib window!', 120, 200, 20, r.BLACK)
  r.EndDrawing()
}
r.CloseWindow()
...
/*
 * REQUIRE HELPER/WRAPPER
 *
 * This version specifies the `fresh_compile_up_to_date.node` DLL when running .dlopen
 *  
 * The .node DLL exports Bind functions to the raylib native functions, but
 * there's a bunch of extra js handling over top of these that the node-raylib package
 * does to make it friendly for the developer.
 * 
 * This function is in place of where the developer would normally require the wrapper,
 * since everything needs to be bundled in one file.
 * 
 * I've chosen to put this below the contents of actually utilizing the node-raylib package,
 * relying on hoisting, so that the main example content can be easily seen.
 */
function requireFreshCompileNodeRaylibWrapper() {
    /**
     * node-raylib
     *
     * @module raylib
    ...
    const importmodule = { exports: {} };

    console.log('Before DLL `dlopen`')

    process.dlopen(
        importmodule, 
        path.join(__dirname, '..', 'dlls', 'fresh_compile_up_to_date.node'),
        constants.dlopen.RTLD_NOW
    );

    console.log('After DLL `dlopen`')

    const r = importmodule.exports;
